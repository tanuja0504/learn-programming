    ...
    public static int minSubArrayLen(int target, int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (sum >= target) {
                ...
                min = Math.min(min, high - i);
                tSum = sum;

            }
            sum = sum - nums[i];
            System.out.println(Math.abs(sum - target));
            if (sum < target && high >= nums.length) {
                break;
            }
        }
        return (tSum == Integer.MAX_VALUE) ? 0 : min;
    }
